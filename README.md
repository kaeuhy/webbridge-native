...
| mock 응답 DevTools 표시 | **Service Worker** | 없음 |
...
**WebBridge Native**: 쿠키 + 캐시 + 리다이렉트 + 헤더 + mock을 **하나의 라이브러리로 통합**. Native bridge로 **DevTools에서 mock까지 표시**. RN에서 브라우저처럼.
...
| [`@webbridge-native/native-bridge`](packages/native-bridge) | **Native 브릿지** — iOS/Android DevTools 가시성 (NSURLProtocol + OkHttp) |
...
| [`@webbridge-native/mock`](packages/mock) | MSW v2 호환 mock (native-bridge로 DevTools에서 보임) |
...
# DevTools 가시성이 필요하면 native-bridge 추가
...
const { client } = setupWebBridge({
  ...
  nativeBridge: true, // DevTools Network 탭에 표시 (선택)
});
...
- **React Native 0.73+**
- native-bridge 없이: 순수 JS/TS, Expo 호환
- native-bridge 사용 시: iOS 13.4+, Android minSdk 24+

## 아키텍처

```
Layer 4: 개발자 API (preset, mock, adapters)
Layer 3: 브라우저 시맨틱 (cookies, cache, headers, cors, redirect)
Layer 2: 요청 파이프라인 (인터셉터 체인, devtools)
Layer 1: Native 브릿지 (NSURLProtocol / OkHttp — DevTools 가시성)
```

Layer 1~3은 모듈별 opt-in/opt-out 가능. native-bridge 없이도 순수 JS로 동작합니다.
