# 08-TROUBLESHOOTING.md — 알려진 문제 카탈로그 (시드)

> **이 문서는 운영 중 마주칠 모든 문제 패턴의 초기 카탈로그다.**
> 부트스트랩 Step 8에서 이 내용을 `docs/TROUBLESHOOTING.md`로 시드 복사한다.
> 이후 운영 중 새 문제가 해결될 때마다 `/add-troubleshoot-entry`로 자동 확장된다.
